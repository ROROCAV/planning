...
% poly_coef_x = flipud(poly_coef_x);
% poly_coef_y = flipud(poly_coef_y);
...
for k = 1:n_seg
    for t = 0:0.01:1
        ...
        for i = 0:n_order
            ...
            x_pos(idx) = basis_p * poly_coef_x((n_order+1)*(k-1)+i+1) + x_pos(idx);
            y_pos(idx) = basis_p * poly_coef_y((n_order+1)*(k-1)+i+1) + y_pos(idx);
        end
        idx = idx + 1;
    end
end
...
function poly_coef = MinimumSnapCorridorBezierSolver(axis, waypoints, corridor, ts, n_seg, n_order, v_max, a_max)
    ...
    %                                   ...,
    %                                   pn_min, pn_max ];
    corridor_range = zeros(2,n_seg);
    if axis == 1
        % upper boundary
        corridor_range(1,:) = corridor(1,:) + corridor(3,:); 
        % lower boundary
        corridor_range(2,:) = corridor(1,:) - corridor(3,:);
    elseif axis == 2
        corridor_range(1,:) = corridor(2,:) + corridor(4,:);
        corridor_range(2,:) = corridor(2,:) - corridor(4,:);
    end
    % corridor_range
